...
def eos_log_handler():
    log_backup_command = str(work_home) + '/node/backup_log.sh'
    subprocess.call(log_backup_command)


def data_backup_handler():
    data_backup_command = str(work_home) + '/backup/backup_for_cron.sh'
